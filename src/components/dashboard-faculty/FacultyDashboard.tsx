...
export default function FacultyDashboard({ onLogout }: FacultyProps) {
  ...
  if (!mounted)
    return <div className="h-screen w-full bg-slate-50 dark:bg-slate-950" />;

  return (
    <div
      className={`flex h-screen w-full font-sans overflow-hidden transition-colors duration-200 ease-in-out ${theme.bgMain} ${theme.textMain}`}
    >
      ...
      >
        <div
          className={`h-16 flex items-center px-5 border-b shrink-0 ${theme.border} ${isMobile ? "justify-between" : ""}`}
        >
          <div className="flex items-center overflow-hidden">
            ...
            <div
              className={`ml-3 font-bold text-lg tracking-tight whitespace-nowrap transition-all duration-300 ${!isMobile && !isSidebarOpen ? "opacity-0 w-0" : "opacity-100 w-auto"}`}
            >
              ARCADE{" "}
          ...
          {MODULES.map((item) => {
            ...
            return (
              ...
              >
                ...
                />
                <span
                  className={`ml-3 transition-opacity duration-300 ${!isMobile && !isSidebarOpen ? "opacity-0" : "opacity-100"}`}
                >
                  {item.label}
                </span>
              </button>
  ...
  <button 
    onClick={onLogout} 
    className={`w-full flex items-center px-3 py-2 rounded-md text-sm font-medium text-red-600 hover:bg-red-50 dark:hover:bg-red-900/10 transition-colors whitespace-nowrap overflow-hidden`}
  >
    ...
    <span className={`ml-3 transition-opacity duration-300 ${!isMobile && !isSidebarOpen ? "opacity-0" : "opacity-100"}`}>Sign Out</span>
      ...
      <div className="flex-1 flex flex-col h-full min-w-0 relative">
        <header
          className={`h-16 border-b flex items-center justify-between px-4 md:px-6 z-30 shrink-0 ${theme.bgSidebar} ${theme.border}`}
        >
          <div className="flex items-center gap-3">
            <button
              onClick={() => setIsSidebarOpen(!isSidebarOpen)}
              className={`p-2 rounded-md transition-colors ${theme.hover} ${theme.textSub}`}
            >
              <Menu size={20} />
            </button>
            <h1 className="text-lg font-semibold capitalize tracking-tight truncate max-w-[120px] md:max-w-none">
          ...
          <div className="flex items-center gap-2 md:gap-4">
            ...
            <div className="relative">
              <button
                ...
                className={`p-2 rounded-md relative transition-colors ${showNotifPanel ? "text-blue-600 bg-blue-50 dark:bg-blue-900/20" : `${theme.textSub} ${theme.hover}`}`}
              >
                <Bell size={20} />
                  ...
                  <>
                    ...
                    >
                      ...
                      <div className="max-h-[60vh] overflow-y-auto custom-scrollbar">
                        {notifications.length === 0 ? (
                          <div
                            className={`p-8 text-center flex flex-col items-center ${isDarkMode ? "text-slate-500" : "text-slate-400"}`}
                          >
                          ...
                          notifications.map((n) => (
                            ...
                            >
                              ...
                              <div className="flex gap-3">
                                <div
                                  className={`mt-0.5 shrink-0 ${n.type === "warning" ? "text-amber-500" : n.type === "success" ? "text-emerald-500" : n.type === "error" ? "text-red-500" : "text-blue-500"}`}
                                >
                                  {n.type === "warning" ? (
                                ...
                                <div className="min-w-0 pr-4">
                                  <h4
                                    className={`text-sm font-semibold leading-tight truncate ${theme.textMain}`}
                                  >
                                    {n.title}
                                  </h4>
                                  <p
                                    className={`text-xs mt-1 break-words leading-relaxed ${theme.textSub}`}
                                  >
                                    {n.desc}
                                  </p>
                                  <span
                                    className={`text-[10px] mt-1.5 block ${theme.textSub} opacity-70`}
                                  >
                                    {n.time}
                                  </span>
                                </div>
            ...
            <button
              onClick={toggleTheme}
              className={`p-2 rounded-md border transition-all ${isDarkMode ? "bg-slate-800 border-slate-700 text-yellow-400" : "bg-white border-slate-200 text-slate-600"}`}
            >
        ...
        <main
          className={`flex-1 overflow-y-auto p-4 md:p-8 transition-colors duration-200 ${theme.bgMain}`}
        >
