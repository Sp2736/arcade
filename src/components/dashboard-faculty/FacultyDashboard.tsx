...
export default function FacultyDashboard() {
  ...
  return (
    <div className={`flex h-screen w-full font-sans overflow-hidden transition-colors duration-300 ${theme.bgMain} ${theme.textMain}`}>
      ...
      {/* SIDEBAR */}
      <aside className={`flex-shrink-0 border-r transition-all duration-300 flex flex-col z-20 ${theme.bgSidebar} ${theme.border} ${isSidebarOpen ? "w-64" : "w-20"}`}>
        ...
        <nav className="flex-1 overflow-y-auto py-6 px-3 space-y-1">
          {MODULES.map((item) => {
            ...
            return (
              <button key={item.id} onClick={() => setCurrentView(item.id)} title={!isSidebarOpen ? item.label : ""} className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-md text-sm font-medium transition-all group whitespace-nowrap ${isActive ? theme.activeNav : `${theme.inactiveNav} ${theme.hover}`}`}>
      ...
      <div className="flex-1 flex flex-col h-full min-w-0 relative">
        <header className={`h-16 border-b flex items-center justify-between px-6 z-30 ${theme.bgSidebar} ${theme.border}`}>
          <div className="flex items-center gap-4">
            <button onClick={() => setIsSidebarOpen(!isSidebarOpen)} className={`p-2 rounded-md ${theme.hover} ${theme.textSub}`}><Menu size={20} /></button>
            <h1 className="text-lg font-semibold capitalize tracking-tight">{MODULES.find(m => m.id === currentView)?.label}</h1>
          </div>
          
          <div className="flex items-center gap-4">
            <div className="relative">
                <button onClick={() => setShowNotifPanel(!showNotifPanel)} className={`p-2 rounded-md relative ${showNotifPanel ? "text-blue-600 bg-blue-50 dark:bg-blue-900/20" : `${theme.textSub} ${theme.hover}`}`}>
                    <Bell size={20} />
                ...
                <AnimatePresence>
                    {showNotifPanel && (
                        <motion.div initial={{ opacity: 0, y: 10, scale: 0.95 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0, y: 10, scale: 0.95 }} className={`absolute right-0 top-12 w-80 rounded-md border shadow-xl overflow-hidden z-50 ${isDarkMode ? "bg-slate-900 border-slate-800" : "bg-white border-slate-200"}`}>
            ...
            <button onClick={() => setIsDarkMode(!isDarkMode)} className={`p-2 rounded-md border transition-colors ${isDarkMode ? "bg-slate-800 border-slate-700 text-yellow-400" : "bg-white border-slate-200 text-slate-600"}`}>{isDarkMode ? <Sun size={18} /> : <Moon size={18} />}</button>
