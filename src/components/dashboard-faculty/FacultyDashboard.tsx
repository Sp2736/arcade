...
export default function FacultyDashboard() {
  ...
  useEffect(() => {
    ...
    setMounted(true); // UI is ready to render
  }, []);
  ...
  useEffect(() => {
    setNotifications([
        { id: 1, title: "Pending Approvals", desc: "5 new notes require your verification.", time: "10m ago", type: "warning" },
    ]);
  }, []);
  ...
  return (
    <div className={`flex h-screen w-full font-sans overflow-hidden transition-colors duration-200 ease-in-out ${theme.bgMain} ${theme.textMain}`}>
      ...
      {/* SIDEBAR */}
      <aside className={`flex-shrink-0 border-r flex flex-col z-20 transition-[width,colors] duration-300 ease-in-out ${theme.bgSidebar} ${theme.border} ${isSidebarOpen ? "w-64" : "w-20"}`}>
        <div className={`h-16 flex items-center px-6 border-b transition-colors duration-200 ${theme.border}`}>
          ...
          {isSidebarOpen && <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="ml-3 font-bold text-lg tracking-tight whitespace-nowrap">ARCADE <span className="text-xs font-normal text-slate-500 ml-1">ADMIN</span></motion.div>}
        </div>
        ...
        <nav className="flex-1 overflow-y-auto py-6 px-3 space-y-1">
          {MODULES.map((item) => {
            ...
            return (
              <button key={item.id} onClick={() => setCurrentView(item.id)} title={!isSidebarOpen ? item.label : ""} className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-md text-sm font-medium transition-all duration-200 group whitespace-nowrap ${isActive ? theme.activeNav : `${theme.inactiveNav} ${theme.hover}`}`}>
        ...
        <div className={`p-4 border-t transition-colors duration-200 ${theme.border}`}>
          <button className={`w-full flex items-center gap-3 px-3 py-2 rounded-md text-sm font-medium text-red-600 hover:bg-red-50 dark:hover:bg-red-900/10 transition-colors duration-200 whitespace-nowrap`}>
      ...
      <div className="flex-1 flex flex-col h-full min-w-0 relative">
        ...
        {/* TOPBAR */}
        <header className={`h-16 border-b flex items-center justify-between px-6 z-30 transition-colors duration-200 ${theme.bgSidebar} ${theme.border}`}>
          <div className="flex items-center gap-4">
            <button onClick={() => setIsSidebarOpen(!isSidebarOpen)} className={`p-2 rounded-md transition-colors duration-200 ${theme.hover} ${theme.textSub}`}><Menu size={20} /></button>
            <h1 className="text-lg font-semibold capitalize tracking-tight">{MODULES.find(m => m.id === currentView)?.label}</h1>
          </div>
          
          <div className="flex items-center gap-4">
            {/* Notifications */}
            <div className="relative">
                <button onClick={() => setShowNotifPanel(!showNotifPanel)} className={`p-2 rounded-md relative transition-colors duration-200 ${showNotifPanel ? "text-blue-600 bg-blue-50 dark:bg-blue-900/20" : `${theme.textSub} ${theme.hover}`}`}>
                    <Bell size={20} />
                ...
                <AnimatePresence>
                    {showNotifPanel && (
                        <motion.div initial={{ opacity: 0, y: 10, scale: 0.95 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0, y: 10, scale: 0.95 }} className={`absolute right-0 top-12 w-80 rounded-md border shadow-xl overflow-hidden z-50 ${theme.panelBg} ${theme.border}`}>
                            ...
                            <div className="max-h-64 overflow-y-auto">
                                {notifications.length === 0 ? <div className={`p-6 text-center text-xs ${isDarkMode ? "text-slate-500" : "text-slate-400"}`}>No new alerts</div> : notifications.map(n => (
                                    <div key={n.id} className={`p-3 border-b relative group transition-colors ${isDarkMode ? "border-slate-800 hover:bg-slate-800/50" : "border-slate-100 hover:bg-slate-50"}`}>
                                        ...
                                        <div className="flex gap-3">
                                            <div className={`mt-0.5 ${n.type === "warning" ? "text-amber-500" : n.type === "success" ? "text-emerald-500" : n.type === "error" ? "text-red-500" : "text-blue-500"}`}>
                                                {n.type === "warning" ? <AlertTriangle size={14} /> : n.type === "success" ? <CheckCircle size={14} /> : <Info size={14} />}
                                            </div>
                                            <div><h4 className={`text-sm font-semibold leading-tight ${theme.textMain}`}>{n.title}</h4><p className={`text-xs mt-1 ${theme.textSub}`}>{n.desc}</p><span className={`text-[10px] mt-1 block ${theme.textSub}`}>{n.time}</span></div>
                                        </div>
                                    </div>
                                ))}
                            </div>
            ...
            <div className={`h-6 w-px transition-colors duration-200 ${theme.border}`}></div>
            
            {/* Theme Toggle */}
            <button onClick={toggleTheme} className={`p-2 rounded-md border transition-all duration-200 ${isDarkMode ? "bg-slate-800 border-slate-700 text-yellow-400" : "bg-white border-slate-200 text-slate-600"}`}>
                {isDarkMode ? <Sun size={18} /> : <Moon size={18} />}
            </button>
            
            {/* Profile Trigger */}
            <button onClick={() => setCurrentView("profile")} className={`flex items-center gap-3 pl-2 group rounded-md p-1 transition-colors duration-200 ${theme.hover}`}>
                <div className="text-right hidden sm:block"><p className={`text-sm font-bold leading-none ${theme.textMain}`}>Prof. J. Doe</p><p className={`text-[10px] uppercase font-bold ${theme.textSub}`}>HOD - Comp Eng</p></div>
                <div className="h-9 w-9 rounded-md bg-blue-100 text-blue-700 flex items-center justify-center font-bold text-sm border border-blue-200 group-hover:ring-2 ring-blue-500 ring-offset-2 dark:ring-offset-slate-900 transition-all">JD</div>
            </button>
          </div>
        </header>
