...
export default function FacultyDashboard({ user, onLogout }: FacultyProps) {
  ...
  const roleTitle = user?.is_hod ? `HOD - ${department}` : `Faculty - ${department}`;
  ...
  useEffect(() => {
    ...
    const handleResize = () => {
      if (window.innerWidth < 1024) { setIsMobile(true); setIsSidebarOpen(false); } 
    ...
    <div className={`flex h-screen w-full font-sans overflow-hidden transition-colors duration-200 ease-in-out ${theme.bgMain} ${theme.textMain}`}>
      <AnimatePresence>
        {isMobile && isSidebarOpen && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-40 bg-black/60 backdrop-blur-sm" onClick={() => setIsSidebarOpen(false)}/>
        )}
      </AnimatePresence>

      <aside className={`fixed md:relative z-50 h-full flex flex-col border-r transition-[transform,width] duration-300 ease-[cubic-bezier(0.4,0,0.2,1)] ${theme.bgSidebar} ${theme.border} ${isMobile ? isSidebarOpen ? "translate-x-0 w-72" : "-translate-x-full w-72" : isSidebarOpen ? "translate-x-0 w-64" : "translate-x-0 w-20"}`}>
        ...
        <nav className="flex-1 overflow-y-auto py-6 px-3 space-y-1">
          {MODULES.map((item) => {
            ...
            return (
              <button key={item.id} onClick={() => { setCurrentView(item.id); if (isMobile) setIsSidebarOpen(false); }} title={!isSidebarOpen && !isMobile ? item.label : ""} className={`w-full flex items-center px-3 py-2.5 rounded-md text-sm font-medium transition-all duration-200 group whitespace-nowrap overflow-hidden ${isActive ? theme.activeNav : `${theme.inactiveNav} ${theme.hover}`}`}>
        ...
        <div className={`p-4 border-t ${theme.border}`}>
          <button onClick={onLogout} className={`w-full flex items-center px-3 py-2 rounded-md text-sm font-medium text-red-600 hover:bg-red-50 dark:hover:bg-red-900/10 transition-colors whitespace-nowrap overflow-hidden`}>
          ...
          <div className="flex items-center gap-2 md:gap-4">
            <div className="relative">
              ...
              <AnimatePresence>
                {showNotifPanel && (
                  <>
                    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="md:hidden fixed inset-0 z-40 bg-black/20 backdrop-blur-sm" onClick={() => setShowNotifPanel(false)} />
                    <motion.div initial={{ opacity: 0, y: 10, scale: 0.95 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0, y: 10, scale: 0.95 }} className={`fixed md:absolute top-20 md:top-12 left-4 right-4 md:left-auto md:right-0 w-auto md:w-80 rounded-lg border shadow-2xl overflow-hidden z-50 ${theme.panelBg} ${theme.border}`}>
                      ...
                      <div className="max-h-[60vh] overflow-y-auto custom-scrollbar">
                        {notifications.length === 0 ? (
                          <div className={`p-8 text-center flex flex-col items-center ${isDarkMode ? "text-slate-500" : "text-slate-400"}`}>
                            <Bell size={32} className="mb-2 opacity-20" /><span className="text-xs">No new alerts</span>
                          </div>
                        ) : (
                          notifications.map((n) => (
                            <div key={n.notification_id} className={`p-3 border-b relative group transition-colors ${isDarkMode ? "border-slate-800 hover:bg-slate-800/50" : "border-slate-100 hover:bg-slate-50"}`}>
                              ...
                              <div className="flex gap-3">
                                ...
                                <div className="min-w-0 pr-4">
                                  ...
                                  <p className={`text-xs mt-1 break-words leading-relaxed ${theme.textSub}`}>{n.message}</p>
                                  <span className={`text-[10px] mt-1.5 block ${theme.textSub} opacity-70`}>{new Date(n.created_at).toLocaleDateString()}</span>
                                </div>
                              </div>
                            </div>
                          ))
            ...
            <button onClick={() => setCurrentView("profile")} className={`flex items-center gap-3 pl-2 group rounded-md p-1 transition-colors ${theme.hover}`}>
              <div className="text-right hidden md:block">
                <p className={`text-sm font-bold leading-none group-hover:text-blue-600 transition-colors ${theme.textMain}`}>{firstName}</p>
                <p className={`text-[10px] uppercase font-bold ${theme.textSub}`}>{roleTitle}</p>
              </div>
            ...
            <AnimatePresence mode="wait">
              {currentView === "profile" && (
                <motion.div key="profile" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} transition={{ duration: 0.2 }}>
                  <FacultyProfile isDark={isDarkMode} user={user} />
                </motion.div>
              )}
              {currentView === "verification" && (
                <motion.div key="verification" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} transition={{ duration: 0.2 }}>
                  <FacultyVerification isDark={isDarkMode} user={user} />
                </motion.div>
              )}
              {currentView === "uploads" && (
