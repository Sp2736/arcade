    ...
    <div className="h-full flex flex-col space-y-6">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          ...
          <p className={textSub}>
            {activeTab === "resumes" 
                ? "Verified formats classified by experience level." 
        ...
        {activeTab === "resources" && !selectedSubject && (
            <div className={`flex items-center gap-2 px-3 py-2 rounded-xl border transition-colors ${dropdownBorder}`}>
                ...
                <select 
                    ...
                    className={`bg-transparent border-none outline-none text-sm font-bold cursor-pointer ${textMain}`}
                >
      ...
      <div className="flex-1 overflow-y-auto custom-scrollbar pr-2 pb-10 relative">
        ...
        {activeTab === "resumes" ? (
            ...
            // === RESUME ARCHIVES LAYOUT ===
            <div className="space-y-10">
                
                <ResumeSection 
                    title="Internship Opportunities" 
                    icon={GraduationCap} 
                    data={getResumesByLevel("intern")} 
                    isDark={isDark} 
                    color="text-blue-500"
                />
                ...
                <ResumeSection 
                    ...
                    icon={Briefcase} 
                    data={getResumesByLevel("fresher")} 
                    isDark={isDark} 
                ...
                <ResumeSection 
                    ...
                    icon={Award} 
                    data={getResumesByLevel("experienced")} 
                    isDark={isDark} 
                ...
                <ResumeSection 
                    ...
                    icon={Star} 
                    data={getResumesByLevel("advanced")} 
                    isDark={isDark} 
                ...
                {!selectedSubject ? (
                    ...
                    >
                        {filteredSubjects.length > 0 ? filteredSubjects.map((sub) => (
                            ...
                            <div className={`col-span-full p-12 text-center border border-dashed rounded-2xl ${isDark ? "border-white/10 text-zinc-500" : "border-zinc-300 text-zinc-400"}`}>
                                <p>No subjects found for {semesterFilter}.</p>
                            </div>
                        )}
                    </motion.div>
                ) : (
                    <motion.div 
                        ...
                        initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: 20 }}
                    >
                        <button onClick={() => setSelectedSubject(null)} className={`flex items-center gap-2 mb-6 text-sm font-bold transition-colors ${isDark ? "text-zinc-400 hover:text-white" : "text-zinc-600 hover:text-black"}`}>
        ...
        <div className="flex items-center justify-between pt-4 border-t border-dashed border-zinc-700/20">
            ...
            <div className="flex gap-2">
                ...
                <button 
                    ...
                    className={`p-2 rounded-lg transition-colors ${isDark ? "bg-white/5 hover:bg-white/10 text-zinc-400 hover:text-white" : "bg-zinc-100 hover:bg-zinc-200 text-zinc-600 hover:text-black"}`}
                >
                    <Eye size={16} />
                </button>
                ...
                <button 
                    title="Download PDF"
                    className={`p-2 rounded-lg transition-colors ${isElite ? "bg-purple-600 hover:bg-purple-500 text-white" : (isDark ? "bg-white/5 hover:bg-white/10 text-blue-400" : "bg-blue-50 hover:bg-blue-100 text-blue-600")}`}
                >
                    <Download size={16} />
                </button>
            </div>
...
const SubjectCard = ({ data, isDark, onClick }: any) => {
    ...
    return (
        <div onClick={onClick} className={`group relative overflow-hidden p-5 rounded-2xl border transition-all hover:scale-[1.02] cursor-pointer ${isDark ? "bg-zinc-900/40 border-white/5 hover:border-white/10" : "bg-white border-zinc-200 shadow-sm hover:shadow-md"}`}>
            ...
            <div className="relative z-10">
                <div className="flex justify-between items-start mb-4">
                    ...
                    <span className={`text-[10px] font-bold uppercase tracking-wider px-2 py-1 rounded ${isDark ? "bg-white/5 text-zinc-400" : "bg-zinc-100 text-zinc-600"}`}>{data.semester}</span>
                </div>
