...
export default function StudentDashboard({
  ...
  const collegeId = activeUser?.college_id
    ? activeUser.college_id.toUpperCase()
    : "ID PENDING";
  ...
  const currentRoleData =
    ROADMAP_DATA[targetRole] || ROADMAP_DATA["Frontend Developer"];
  const mandatoryDone = currentRoleData.mandatory.every((s: string) =>
    checkedSkills.includes(s),
  );
  const advancedCount = currentRoleData.advanced.filter((s: string) =>
    checkedSkills.includes(s),
  ).length;
    ...
    const fetchProgress = async () => {
      ...
      try {
        ...
        if (res.ok) {
          ...
          if (data.completed_nodes) setCheckedSkills(data.completed_nodes);
        }
      } catch (error) {
  ...
  useEffect(() => {
    if (!progressLoaded || !session?.user) return;
    ...
    return () => clearTimeout(timer);
  }, [checkedSkills, targetRole, progressLoaded, session]);
  ...
  return (
    ...
    >
      ...
      <div className="flex-1 flex flex-col h-full relative z-20 overflow-hidden">
        <header
          className={`h-16 md:h-20 border-b flex items-center justify-between px-4 md:px-8 relative z-50 transition-colors duration-300 ${borderMain}`}
        >
          <div
            className={`absolute inset-0 -z-10 backdrop-blur-md transition-colors duration-300 ${isDarkMode ? "bg-[#050505]/80" : "bg-white/80"}`}
          />

          <div className="flex items-center gap-3 md:gap-4">
            <button
              onClick={() => setMobileMenuOpen(true)}
              className="md:hidden p-2 -ml-2 text-zinc-500 hover:text-zinc-800 transition-colors"
            >
              <Menu size={24} />
            </button>
            <div>
          ...
          <div className="flex items-center gap-2 md:gap-6">
            <button
              ...
              className={`p-2 rounded-full transition-all ${isDarkMode ? "bg-zinc-800 text-yellow-400" : "bg-zinc-100 text-zinc-600"}`}
            >
              ...
              <AnimatePresence>
                {showNotifPanel && (
                  <>
                    ...
                    >
                      <div
                        className={`p-4 border-b flex justify-between items-center ${isDarkMode ? "border-zinc-800 bg-zinc-900" : "border-zinc-100 bg-white"}`}
                      >
                        ...
                        <div className="flex gap-3 items-center">
                          <button
                            onClick={() => setShowNotifPanel(false)}
                            className={`p-1 rounded transition-colors ${isDarkMode ? "hover:bg-zinc-800 text-zinc-400" : "hover:bg-zinc-200 text-zinc-600"}`}
                          >
                            <X size={14} />
                          </button>
                        </div>
                      ...
                      <div className="max-h-[60vh] md:max-h-[400px] overflow-y-auto custom-scrollbar">
                        {notifications.length === 0 ? (
                          <div
                            className={`p-8 text-center text-xs py-12 ${isDarkMode ? "text-zinc-500" : "text-zinc-400"}`}
                          >
                          ...
                          notifications.map((n) => (
                            <div
                              key={n._id}
                              className={`p-4 border-b relative group transition-colors ${isDarkMode ? "border-zinc-800 hover:bg-zinc-800/50" : "border-zinc-100 hover:bg-zinc-50"}`}
                            >
                              <button
                              ...
                              <div className="flex gap-3">
                                <div
                                  className={`mt-0.5 shrink-0 ${n.type === "success" ? "text-green-500" : n.type === "error" ? "text-red-500" : n.type === "warning" ? "text-amber-500" : "text-blue-500"}`}
                                >
                                  {n.type === "success" ? (
                                    <CheckCircle size={16} />
                                  ) : n.type === "error" ||
                                ...
                                <div className="min-w-0 pr-4">
                                  <h4
                                    className={`text-sm font-bold leading-tight mb-1 truncate ${textMain}`}
                                  >
                                    {n.title}
                                  </h4>
                                  <p
                                    className={`text-xs leading-relaxed break-words ${isDarkMode ? "text-zinc-400" : "text-zinc-500"}`}
                                  >
                                    {n.message}
                                  </p>
            ...
            <button
              ...
              className="flex items-center gap-3 pl-3 md:pl-6 border-l border-white/10 group"
            >
        ...
        <div className="flex-1 overflow-y-auto custom-scrollbar relative p-4 md:p-10 pb-24 z-0">
          <AnimatePresence mode="wait">
            ...
            >
              ...
              {currentView === "notes" && (
                <NotesView isDark={isDarkMode} user={user} />
              )}
              {currentView === "skills" && <SkillNavigator />}
              {currentView === "resumes" && (
                <ResumeResourcesView isDark={isDarkMode} initialTab="resumes" />
              )}
              ...
              {currentView === "alumni" && (
                <AlumniNetwork isDark={isDarkMode} />
              )}
            </motion.div>
          </AnimatePresence>
        </div>
