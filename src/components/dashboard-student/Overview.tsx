...
export default function Overview({ isDark }: { isDark: boolean }) {
    ...
    <motion.div variants={containerVar} initial="hidden" animate="show" className="space-y-6 pb-10">
      ...
      <motion.div variants={itemVar} className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-zinc-500">Target: <span className="text-blue-500 font-bold">FULL STACK DEV</span></p>
        </div>
        <div className={`px-4 py-2 rounded-lg border flex items-center gap-3 ${isDark ? "bg-blue-900/20 border-blue-500/20 text-blue-400" : "bg-blue-50 border-blue-200 text-blue-700"}`}>
            <Award size={18} /> <span className="font-bold text-sm">Skill Score: 68</span>
        </div>
      </motion.div>

      {/* Action Cards (Grid: 1 -> 3) */}
      ...
      {/* Split Section (Grid: Stacked -> Side-by-Side) */}
      <motion.div variants={itemVar} className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className={`col-span-1 lg:col-span-2 rounded-3xl p-6 border flex flex-col ${cardBg}`}>
            <div className="flex justify-between items-center mb-8">
                ...
                <span className="text-[10px] font-bold bg-blue-500/10 text-blue-500 px-2 py-1 rounded">PHASE 2</span>
            </div>
            
            {/* Responsive Nodes: Flex-wrap on mobile, no wrap on desktop */}
            <div className="flex-1 flex flex-wrap md:flex-nowrap items-center justify-between gap-4 relative">
                ...
                {["HTML", "JS", "React", "Next", "Deploy"].map((l, i) => (
...
const ActionCard = ({ title, desc, icon: Icon, color, isDark }: any) => {
    ...
    return (
        <div className={`p-5 rounded-2xl border transition-all hover:scale-[1.02] active:scale-[0.98] ${isDark ? "bg-[#09090b]/40 border-white/5" : "bg-white border-zinc-200 shadow-sm"}`}>
...
const ResourceItem = ({ title, tag, type, isDark }: any) => (
    <div className={`flex items-center gap-3 p-3 rounded-xl border ${isDark ? "bg-white/5 border-white/5" : "bg-zinc-50 border-zinc-200"}`}>
