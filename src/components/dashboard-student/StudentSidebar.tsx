...
export default function StudentSidebar({ activeView, onNavigate, isDarkMode }: SidebarProps) {
    ...
    <div className={`h-full w-20 md:w-64 backdrop-blur-xl border-r flex flex-col justify-between py-6 transition-all duration-300 ${containerClass}`}>
      ...
      <div className="px-0 md:px-8 flex flex-col items-center md:items-start mb-8">
        ...
        <div className="hidden md:block mt-4">
          ...
          <p className={`text-[10px] font-mono ${textSub}`}>CAREER TERMINAL</p>
        </div>
      </div>
        ...
        {MENU_ITEMS.map((item) => {
          ...
          if (isActive) {
            buttonClass = isDarkMode 
                ...
                : "bg-blue-50 text-blue-700"; // Darker blue for light mode
          } else {
            buttonClass = isDarkMode
                ? "text-zinc-500 hover:text-white hover:bg-white/5"
                : "text-zinc-500 hover:text-black hover:bg-zinc-200/70"; // Higher contrast hover
          }
