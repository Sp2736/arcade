    ...
    <div className="flex flex-col h-full p-6">
       ...
       <div className="flex flex-col items-start mb-10">
          <div className="flex items-center justify-between w-full">
            <div className="w-10 h-10 bg-blue-600 rounded-xl flex items-center justify-center text-white font-black text-xl shadow-lg shadow-blue-600/20">A</div>
            <button onClick={onMobileClose} className="md:hidden p-2 text-zinc-500 hover:text-zinc-800"><X size={20} /></button>
          </div>
          <div className="mt-4">
             ...
             <button
               ...
               className={`w-full flex items-center gap-4 p-3 rounded-xl transition-all duration-200 group relative ${
                 isActive 
                  ...
                  : (isDarkMode ? "text-zinc-500 hover:text-white hover:bg-white/5" : "text-zinc-500 hover:text-black hover:bg-zinc-100/70")
               }`}
             >
               {isActive && (
                 <motion.div layoutId="sidebar-active" className="absolute left-0 w-1 h-6 bg-blue-500 rounded-r-full" />
               )}
               <div className="relative"><item.icon size={20} strokeWidth={isActive ? 2.5 : 2} /></div>
  ...
  return (
    <>
        ...
        <AnimatePresence>
            {mobileOpen && (
                <>
                    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} onClick={onMobileClose} className="fixed inset-0 bg-black/60 z-40 md:hidden backdrop-blur-sm"/>
                    <motion.div initial={{ x: "-100%" }} animate={{ x: 0 }} exit={{ x: "-100%" }} transition={{ type: "spring", damping: 25, stiffness: 200 }} className={`fixed inset-y-0 left-0 z-50 w-72 border-r shadow-2xl ${bgClass}`}>
                        <SidebarContent />
                    </motion.div>
                </>
            )}
